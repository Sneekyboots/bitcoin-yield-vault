...
async function main() {
  ...
  // Import ethers directly and hardhat for artifacts
  const { ethers } = await import("ethers");
  const hre = await import("hardhat");
  
  // Start Hardhat node first or connect to existing one
  console.log("Connecting to Hardhat network...");
  
  // Create provider - try localhost first, fall back to hardhat network
  ...
  try {
    ...
    // If localhost fails, we'll need to start hardhat node
    console.log("Please start Hardhat node in another terminal:");
  ...
  const tx1 = await crossChainYield.setAuthorizedVault(bitcoinYieldVaultAddress, true);
  await tx1.wait();
  ...
  console.log("\\nAdding Compound protocol...");
  const tx2 = await crossChainYield.addProtocol(
  ...
  console.log("\\nAdding Aave protocol...");
  const tx3 = await crossChainYield.addProtocol(
    aaveAdapterAddress,
    "0x028171bCA77440897B824Ca71D1c56caC55b68A3", // aDAI
    1, // Ethereum
    300, // 3% APY
  ...
  console.log("\\nAdding PancakeSwap protocol...");
  const tx4 = await crossChainYield.addProtocol(
  ...
  console.log("\\nAdding QuickSwap protocol...");
  const tx5 = await crossChainYield.addProtocol(
  ...
  const tx6 = await crossChainYield.setParameters(300, 3); // 3% slippage, 3 max retries
  await tx6.wait();
  console.log("System parameters set");
