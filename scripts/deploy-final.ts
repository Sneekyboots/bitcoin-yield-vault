// Working deployment script using the correct Hardhat 3 pattern
import fs from 'fs';
...
async function main() {
  ...
  // Access ethers through the hardhat-toolbox-mocha-ethers plugin
  // In Hardhat 3, ethers is available on the global scope when using the plugin
  const ethers = (global as any).ethers || hre.ethers;
  
  if (!ethers) {
    throw new Error("Ethers not available. Make sure hardhat-toolbox-mocha-ethers plugin is installed and configured.");
  }
